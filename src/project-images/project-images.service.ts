...
export class ProjectImagesService {
  ...
  async upload(
    ...
    const existing = await this.prisma.projectImage.count({ where: { projectId } });

    return Promise.all(
      files.map(async (file, idx) => {
        const s3Key = await this.s3.uploadFile(file, `projects/${projectId}/images`);
        const url = await this.s3.generateSignedUrl(s3Key);
        return this.prisma.projectImage.create({
          data: { projectId, s3Key, url, caption: captions?.[idx], order: existing + idx },
        });
      }),
    );
  }
  ...
  async update(projectId: string, imageId: string, dto: UpdateImageDto) {
    ...
    return this.prisma.projectImage.update({ where: { id: imageId }, data: dto });
  }
